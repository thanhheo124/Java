...
public class DivisionAB {
	 ...
	 */
	public boolean checkData(String stringData, String name) {
		boolean checkKQ = true;
		if (stringData.length() == 0) {
			System.out.println("Hãy nhập giá trị cho [" + name + "]");
			checkKQ = false;
		} else {
			checkKQ = validateNumber(stringData);
			if (!checkKQ) {
				System.out.println("Giá trị [" + name + "] phải là giá trị số và > 0. Hãy nhập lại");
			} else {
				...
				if (so == 0) {
					...
					System.out.println("Giá trị [" + name + "]không được lớn hơn 5 số. Hãy nhập lại.");
					checkKQ = false;
				}
			}
		}
	 ...
	 * 
	 * @param stringData
	 *            - dữ liệu truyền vào để kiểm tra trả về true, false
	 */
